...
The full in-app **Help** copy is mirrored on the [project wiki](https://github.com/mr-mpage/zoey-tracker/wiki)
(edit-mode and viewer-mode guides + a glossary), useful for evaluating the app
before deploying. Wiki pages are regenerated from `HelpModal.tsx` via
`scripts/publish-wiki.sh` before each release.
